...
export default function ScheduleAppointmentPage() {
  ...
  const [selectedDoctor, setSelectedDoctor] = useState<Id<"users"> | null>(
    null,
  );
  ...
  const parsedRisk = Number(searchParams.get("riskScore") || "0");
  const patientRiskScore = Number.isFinite(parsedRisk)
    ? Math.max(0, Math.min(100, Math.round(parsedRisk)))
    : 0;
  ...
  const handleSchedule = async () => {
    if (!selectedDoctor || !consultationType || !selectedTime || !convexUser)
      return;
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-zinc-500 mt-2">
            Choose a specialist and consultation preference.
          </p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <CardContent className="p-6 flex flex-col justify-center items-center text-center gap-3">
                    <div
                      className={`p-4 rounded-full ${consultationType === "online" ? "bg-primary/20" : "bg-zinc-100 dark:bg-zinc-800"}`}
                    >
                      <Video
                        className={`w-8 h-8 ${consultationType === "online" ? "text-primary" : "text-zinc-500"}`}
                      />
                    </div>
                    <div>
                      <p className="font-semibold text-zinc-900 dark:text-white">
                ...
                >
                  <CardContent className="p-6 flex flex-col justify-center items-center text-center gap-3">
                    <div
                      className={`p-4 rounded-full ${consultationType === "offline" ? "bg-primary/20" : "bg-zinc-100 dark:bg-zinc-800"}`}
                    >
                      <Building2
                        className={`w-8 h-8 ${consultationType === "offline" ? "text-primary" : "text-zinc-500"}`}
                      />
                    </div>
                    <div>
                      <p className="font-semibold text-zinc-900 dark:text-white">
